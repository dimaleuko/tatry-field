window.TATRY_FIELD_MAPS={
  ...
  'kasprowy-gasienicowa-loop':{
    issue:'FIELD MAP 29 / KASPROWY LOOP',title:'Одна вершина. Два совершенно разных пути.',
    intro:'Прямой зелёный набор через Myślenickie Turnie, ветровой порог на Kasprowy, жёлтый спуск в Dolina Gąsienicowa и развилка, которая действительно замыкает кольцо.',
    stops:[
      {ratio:0,kicker:'00 / START',name:'Kuźnice',tag:'ЗЕЛЁНЫЙ НА KASPROWY',meta:'0.0 км',level:'info',prompt:'ПОРЯДОК КОЛЬЦА',decision:'Начинай по зелёной маркировке через Myślenickie Turnie; синюю тропу через Boczań оставь для возвращения.',body:'Направление выбрано специально: длинный набор проходим свежими ногами, а в Dolina Gąsienicowa попадаем уже после вершины.'},
      {ratio:.11,kicker:'01 / JUNCTION',name:'Лес над Kuźnice',tag:'НЕ НА KALATÓWKI',meta:'≈ 1.5 км',level:'nav',prompt:'ПЕРВАЯ ПРОВЕРКА',decision:'Оставайся на зелёной маркировке Kasprowy Wierch, несмотря на боковые тропы и поток к другим целям.',body:'В нижнем лесу соседние маршруты визуально убедительнее карты. Проверь цвет до того, как наберёшь лишние метры.'},
      {ratio:.23,kicker:'02 / MIDPOINT',name:'Myślenickie Turnie',tag:'≈ 3.2 КМ / ЕЩЁ ВВЕРХ',meta:'≈ 3.2 км',level:'effort',prompt:'ТЕМП + ВОДА',decision:'Если группа уже отстаёт от шестичасового графика, вершина и полный круг потребуют сокращения пауз.',body:'Это не середина дистанции, а середина основного набора. Канатка рядом не даёт гарантированного аварийного спуска.'},
      {ratio:.36,kicker:'03 / WEATHER GATE',name:'Открытый верхний склон',tag:'ВЕТЕР + ОБЛАКО',meta:'≈ 5.0 км',level:'danger',prompt:'ПОСЛЕДНИЙ ПРОСТОЙ РАЗВОРОТ',decision:'Если ветер мешает устойчиво идти или вершина исчезла в плотном облаке, разворачивайся по зелёной линии.',body:'Выше укрытий почти нет. Мокрый камень и плохая видимость особенно ухудшат будущий жёлтый спуск.',photoIndex:0},
      {ratio:.465,kicker:'04 / SUMMIT',name:'Kasprowy Wierch',tag:'1987 М',meta:'≈ 6.4 км',level:'summit',prompt:'СМЕНА МАРШРУТА',decision:'Не возвращайся на зелёную тропу: для кольца найди жёлтую маркировку в сторону Sucha Przełęcz и Dolina Gąsienicowa.',body:'Инфраструктура и люди скрывают развилку. Здесь начинается вторая, менее очевидная половина дня.',photoIndex:1},
      {ratio:.52,kicker:'05 / DESCENT',name:'Sucha Przełęcz',tag:'ЖЁЛТЫЙ В ДОЛИНУ',meta:'≈ 7.1 км',level:'nav',prompt:'НЕ ВДОЛЬ ГРАНИЦЫ',decision:'Продолжай по жёлтой маркировке вниз к Dwoiśniak; не уходи дальше по хребту к Liliowe.',body:'В облаке главная линия рельефа зовёт вдоль границы, тогда как нужный маршрут заметно теряет высоту.'},
      {ratio:.60,kicker:'06 / STONES',name:'Спуск в Gąsienicowa',tag:'КАМНИ / БЕЗ ЦЕПЕЙ',meta:'≈ 8.2 км',level:'danger',prompt:'СЦЕПЛЕНИЕ',decision:'Снизь темп на мокрых плитах и не режь серпантины по траве или осыпи.',body:'Цепей нет, но это самый требовательный к постановке ног сектор круга. Остаточный снег полностью меняет категорию маршрута.'},
      {ratio:.66,kicker:'07 / VALLEY',name:'Hala Gąsienicowa / Murowaniec',tag:'ПАУЗА, НЕ ОБЕД',meta:'≈ 9.1 км',level:'info',prompt:'КОНТРОЛЬ 7 ЧАСОВ',decision:'Сверь время: до Kuźnice остаётся около 4.6 км. Для короткого лимита ограничь остановку и готовь синюю линию через Boczań.',body:'Это самый удобный пит-стоп, но 6:20–6:30 в карточке — ходовое время без длинной очереди в приюте.',photoIndex:2},
      {ratio:.75,kicker:'08 / DECISION',name:'Przełęcz między Kopami',tag:'СИНИЙ ЧЕРЕЗ BOCZAŃ',meta:'≈ 10.3 км',level:'nav',prompt:'КАК ЗАМКНУТЬ КРУГ',decision:'Выбирай синюю маркировку через Skupniów Upłaz и Boczań. Жёлтая через Jaworzynka — другой допустимый, но не этот трек.',body:'Здесь две тропы ведут в Kuźnice. Если цель — совпасть с картой и фотографиями, цвет и название варианта важны.'},
      {ratio:.86,kicker:'09 / RIDGE',name:'Skupniów Upłaz / Boczań',tag:'ПОСЛЕДНИЙ ВИД',meta:'≈ 11.8 км',level:'effort',prompt:'КОЛЕНИ + ПОГОДА',decision:'Следуй синей маркировке до Kuźnice и не срезай по локальным лесным следам.',body:'Открытый гребень даёт финальный вид на Gąsienicowa, после чего начинается длинный лесной спуск.'},
      {ratio:1,kicker:'10 / FINISH',name:'Kuźnice',tag:'КОЛЬЦО ЗАМКНУТО',meta:'≈ 13.8 км',level:'info',prompt:'ФИНИШ',decision:'Маршрут заканчивается у того же транспортного узла, но ни один участок зелёного подъёма не повторён.',body:'Если итоговое время меньше семи часов, это был плотный день с короткими остановками — именно так он и спроектирован.'}
    ],
    terrain:[{from:0,to:.23,kind:'approach'},{from:.23,to:.465,kind:'effort'},{from:.465,to:.60,kind:'navigation'},{from:.60,to:.66,kind:'technical'},{from:.66,to:.75,kind:'approach'},{from:.75,to:.86,kind:'navigation'},{from:.86,to:1,kind:'approach'}],
    sources:[['TPN current conditions','https://tpn.gov.pl/komunikat-turystyczny'],['route reference','https://mapa-turystyczna.pl/route/3gm5z'],['Hala Gąsienicowa trail context','https://tpn.gov.pl/szlaki-turystyczne/dolina-suchej-wody-hala-gasienicowa-czarny-staw-gasienicowy-przelecz-miedzy-kopami-boczan-kuznice']]
  }
};
