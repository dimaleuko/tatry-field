(function(){
  ...
  function bestFor(value=''){
    ...
    return source.replace('май','May').replace('июнь','June').replace('июль','July').replace('август','August').replace('сентябрь','September').replace('октябрь','October').replace('зима','winter').replace('при сухих стабильных условиях','in dry, stable conditions').replace('при сухой скале','with dry rock').replace('только сухая устойчивая погода','only in dry, stable weather').replace('при сухом бесснежном склоне','with a dry, snow-free slope').replace('сухо и без снега','dry and snow-free').replace('при стабильной погоде','in stable weather').replace('в грязный сезон медленнее','slower in muddy seasons');
  }
