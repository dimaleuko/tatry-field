...
function renderSafetyNotice(notice){
  ...
  return `<section class="route-section route-safety-notice ${status}" role="note" aria-label="Важное предупреждение по безопасности"><div class="route-safety-kicker"><span>${esc(notice.label||'SAFETY CHECK')}</span><b>${status==='closed'?'ЗАКРЫТО':'ВЫСОКИЙ РИСК'}</b></div><div class="route-safety-copy"><h2>${esc(notice.title)}</h2><p>${esc(notice.text)}</p></div><ol>${(notice.checks||[]).map((item,index)=>`<li><span>${String(index+1).padStart(2,'0')}</span>${esc(item)}</li>`).join('')}</ol>${notice.sourceUrl?`<a target="_blank" rel="noopener noreferrer" href="${esc(notice.sourceUrl)}">ОТКРЫТЬ ОФИЦИАЛЬНЫЙ ИСТОЧНИК ↗</a>`:''}</section>`;
}
function personalRouteMarkup(){
  const fit=tripStore.personalizedDifficulty(route);
  return `<div class="route-personal-fit ${fit.status}"><small>ДЛЯ ТЕБЯ</small><b>${fit.status==='unknown'?'—':`${fit.score}/5`} · ${esc(fit.label)}</b><span>${fit.gaps.length?esc(fit.gaps.join(' · ')):esc(fit.note||'Параметры совпадают с привычной нагрузкой.')}</span>${fit.status==='unknown'?'<a href="/#trip-studio">заполнить профиль →</a>':''}</div>`;
}
...
function renderBase(){
  ...
  const gpxAction=route.temporarilyClosed?'<span class="btn acid disabled" aria-disabled="true" title="GPX недоступен, пока участок официально закрыт">GPX НЕДОСТУПЕН</span>':`<a class="btn acid" href="/api/gpx/${route.id}">Скачать GPX</a>`;
  ...
  <section class="route-hero"><a class="back" href="/">← все маршруты</a><h1>${esc(route.name)}</h1>
    <div class="route-deck"><p>${esc(route.why)}</p><div class="difficulty-stack"><div class="difficulty-box"><small>базовая сложность ${route.diff}/5</small><b>${LEVELS[route.diff]}</b></div>${personalRouteMarkup()}</div></div>
  ...
  <div class="route-body"><div class="route-main">
    ...
    <section class="route-section route-pack-section"><div class="intelligence-head"><div><div class="eyebrow">ROUTE PACK / ТОЛЬКО ДЛЯ ЭТОГО ДНЯ</div><h2>Что положить<br>в этот рюкзак.</h2></div><p>База одинаковая, но длинный день, цепи, набор и открытый рельеф добавляют свои вещи. Отмеченное сохранится на этом устройстве.</p></div><div id="routePack"></div></section>
...
function shortDate(value){try{return new Intl.DateTimeFormat('ru-RU',{weekday:'short',day:'numeric',month:'short'}).format(new Date(`${value}T12:00:00`))}catch{return value||'—'}}
...
function renderMorningBriefing(){
  ...
  if(!weatherData&&!officialData){host.innerHTML='<span class="loading">Собираю прогноз для старта и верхней точки, затем сверяю TPN…</span>';return}
  ...
  const dayButtons=(weatherData?.days||[{day:0,summit:{date:''}},{day:1,summit:{date:''}}]).map((item,index)=>`<button type="button" class="briefing-day${briefingDay===index?' active':''}" data-briefing-day="${index}">${index===0?'СЕГОДНЯ':'ЗАВТРА'}<span>${esc(shortDate(item.summit?.date))}</span></button>`).join('');
  const observationLabels={officialRead:'TPN прочитан полностью',wet:'мокрая скала',snow:'снег / лёд',lowVisibility:'плохая видимость',groupNotReady:'группа не готова'};
  ...
  host.innerHTML=`<div class="briefing-days">${dayButtons}</div><div class="briefing-card ${result.key}"><div class="briefing-verdict"><small>VERDICT / НЕ РАЗРЕШЕНИЕ</small><strong>${esc(result.label)}</strong><p>${result.key==='go'?'Проверки не нашли явной причины менять план. Продолжай оценивать фактические условия и состояние группы.':result.key==='adjust'?'Уменьши амбиции, время или выбери Plan B после ручной проверки.':'Не продолжай высокий вариант с текущими сигналами. Выбирай более низкий план или пропускай выход.'}</p></div><div class="briefing-weather"><small>ВЕРХНЯЯ ТОЧКА / 06—20</small><div><span>Температура<b>${summit?`${summit.minTemp}…${summit.maxTemp}°`:'—'}</b></span><span>Порывы<b>${summit?.maxGust!=null?`${summit.maxGust} км/ч`:'—'}</b></span><span>Осадки<b>${summit?.maxPrecipProb!=null?`${summit.maxPrecipProb}% · ${summit.precipMm} мм`:'—'}</b></span><span>Свет<b>${summit?`${clock(summit.sunrise)}—${clock(summit.sunset)}`:'—'}</b></span></div></div><div class="briefing-reasons"><small>ПОЧЕМУ</small><ul>${result.reasons.length?result.reasons.map(reason=>`<li>${esc(reason)}</li>`).join(''):'<li>Явных ограничивающих сигналов в доступных данных нет.</li>'}</ul></div>${result.alternative?`<a class="briefing-alt" href="/route/${result.alternative.id}"><small>PLAN B / НИЖЕ НАГРУЗКА</small><b>${esc(result.alternative.name)}</b><span>${result.alternative.hours} ч · +${result.alternative.ascent} м · без цепей →</span></a>`:''}</div><div class="condition-check"><div><small>РУЧНАЯ ПРОВЕРКА</small><b>Подтверди bulletin и добавь то, что видишь</b></div><div>${observations}</div></div><div class="briefing-sources"><span>${esc(result.caveat)}</span><a href="https://tpn.gov.pl/komunikat-turystyczny" target="_blank" rel="noopener">TPN ↗</a><a href="https://topr.pl/" target="_blank" rel="noopener">TOPR ↗</a><a href="https://lawiny.topr.pl/" target="_blank" rel="noopener">AVALANCHE ↗</a></div>`;
}
...
function renderRoutePack(){
  ...
  const labels={core:'В рюкзак',wear:'На себя',route:'Добавляет маршрут',morning:'Перед выходом'};
  ...
  host.innerHTML=`<div class="pack-progress"><div><small>PACKED</small><b>${done} / ${items.length}</b></div><div class="pack-progress-track"><i style="width:${items.length?done/items.length*100:0}%"></i></div><button type="button" data-reset-pack>СБРОСИТЬ</button></div><div class="pack-groups">${groups}</div><div class="pack-note"><b>* Критичные проверки и базовые вещи.</b> Список не определяет техническое снаряжение за тебя: зимой и на сложном рельефе нужны отдельные навыки, обучение и актуальная оценка условий.</div>`;
...
async function boot(){const [data,zonesData]=await Promise.all([fetch('/api/routes').then(r=>r.json()),fetch('/api/stay-zones').then(r=>r.json())]);routes=data.routes;stayZones=zonesData.zones||[];route=routes.find(r=>r.id===id);if(!route){document.querySelector('#routeApp').innerHTML='<section class="route-hero"><h1>Маршрут не найден</h1></section>';return}renderBase();await Promise.allSettled([loadOfficial(),loadWeather(),loadPois(),loadStays(),loadTrailPois()])}
