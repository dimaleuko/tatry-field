...
function renderBase(){
  ...
  <div class="route-body"><div class="route-main">
    ...
    <section class="route-section"><h2>Маршрут / GPX</h2><div class="map-toolbar map-toolbar-rich"><div><span id="routeGeometryMeta" class="loading">Строю линию по OSM…</span><span class="context-distance">Старт ≈ ${straight.toFixed(1)} км по прямой от центра Zakopane</span></div><div class="map-toolbar-actions"><div class="map-view-toggle"><button class="active" data-map-view="route">Маршрут</button><button data-map-view="context">Где это?</button></div><a class="btn acid" href="/api/gpx/${route.id}">Скачать GPX</a></div></div><div class="map-frame"><div id="detailMap" class="detail-map"></div><div class="map-legend"><span><i class="legend-start"></i>старт / финиш</span><span><i class="legend-goal"></i>цель / ключевая точка</span><span><i class="legend-route"></i>маршрут</span></div></div><div class="source-note">Планировочная карта показывает контекст. Для движения по маршруту используй Trail Mode: там более детальный zoom и собственная GPS-точка. Линия строится автоматически по OpenStreetMap foot routing через кураторские waypoint’ы и не является официальным GPX TPN.</div></section>
